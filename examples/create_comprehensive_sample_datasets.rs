...
async fn create_business_strategy_dataset(
    ...
    // Create date node
    let _date_id = store
        .create_or_get_date_node(date, Some("Business Strategy Planning Session"))
        .await?;

    // Root strategy node
    let _strategy_id = store
        .create_text_node("# Product Launch Campaign Strategy", Some(date))
        .await?;
...
async fn create_technical_documentation_dataset(
    ...
    let _date_id = store
        .create_or_get_date_node(date, Some("Technical Architecture Documentation"))
        .await?;

    // API Documentation root
    let _api_docs_id = store
        .create_text_node("# NodeSpace API Documentation v2.1", Some(date))
        .await?;
...
async fn create_project_planning_dataset(
    ...
    let _date_id = store
        .create_or_get_date_node(date, Some("Q3 2025 Project Planning Session"))
        .await?;
...
async fn create_research_knowledge_dataset(
    ...
    let _date_id = store
        .create_or_get_date_node(date, Some("AI Research and Knowledge Synthesis"))
        .await?;
...
async fn create_meeting_collaboration_dataset(
    ...
    let _date_id = store
        .create_or_get_date_node(date, Some("Team Collaboration and Meeting Notes"))
        .await?;
...
async fn count_total_nodes(store: &SurrealDataStore) -> Result<usize, Box<dyn std::error::Error>> {
    let text_nodes = store
        .query_nodes("SELECT * FROM text")
        .await
        .unwrap_or_default();
    let date_nodes = store
        .query_nodes("SELECT * FROM date")
        .await
        .unwrap_or_default();
...
async fn test_rag_readiness(store: &SurrealDataStore) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let relationship_test = store
        .get_date_children("2025-06-15")
        .await
        .unwrap_or_default();
    println!(
        "   Hierarchical structure: {} date relationships found",
        relationship_test.len()
    );
