...
use nodespace_data_store::{
    LanceDataStore, DataStore, ImageNode, ImageMetadata, NodeType, 
    HybridSearchConfig, SearchResult
};
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let text_node_1 = Node::with_id(
        ...
        serde_json::Value::String(
            "Claire's birthday party was amazing! I wore my blue shirt and we had cake.".to_string()
        )
    ...
    let text_id_1 = data_store.store_node_with_embedding(text_node_1, text_embedding_1).await?;
    let text_id_2 = data_store.store_node_with_embedding(text_node_2, text_embedding_2).await?;
    ...
    let multimodal_results = data_store.search_multimodal(
        query_embedding.clone(),
        vec![NodeType::Text, NodeType::Image]
    ).await?;
    
    println!("   📊 Found {} cross-modal results:", multimodal_results.len());
    ...
    let image_to_text_results = data_store.search_multimodal(
        image_query_embedding,
        vec![NodeType::Text]
    ).await?;
    
    println!("   📊 Found {} text nodes related to shirt images:", image_to_text_results.len());
    ...
    let hybrid_results = data_store.hybrid_multimodal_search(
        claire_query_embedding,
        &hybrid_config
    ).await?;
    
    println!("   📊 Hybrid search results with relevance scoring:");
    for (i, result) in hybrid_results.iter().enumerate() {
        let preview = result.node.content.as_str()
            .map(|s| if s.len() > 50 { format!("{}...", &s[..47]) } else { s.to_string() })
            .unwrap_or("NULL".to_string());
        
        println!("   {}. Score: {:.3} | {} - {}", 
            i + 1, result.score, result.node.id, preview);
        println!("      Factors: semantic={:.2}, structural={:.2}, temporal={:.2}, cross_modal={:?}",
    ...
    let _perf_results = data_store.hybrid_multimodal_search(
    ...
    if search_duration.as_secs() < 2 {
        ...
        println!("   ⚠️  Performance target missed: {} seconds", search_duration.as_secs());
    }
    ...
    if let Some(retrieved_image) = data_store.get_image_node(&image_id_1).await? {
        println!("   ✅ Successfully retrieved image: {}", retrieved_image.metadata.filename);
        println!("   📏 Dimensions: {}x{}", retrieved_image.metadata.width, retrieved_image.metadata.height);
        println!("   📊 Embedding dimensions: {}", retrieved_image.embedding.len());
        println!("   💾 Image data size: {} bytes", retrieved_image.image_data.len());
        
        if let Some(exif) = &retrieved_image.metadata.exif_data {
            println!("   📸 EXIF data available: {} fields", exif.as_object().unwrap().len());
        }
    }
